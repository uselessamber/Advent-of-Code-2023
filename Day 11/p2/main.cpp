...
int main(){
    ...
    for (int i = 0; i < len(galaxy_pos) - 1; i++){
        for (int j = i + 1; j < len(galaxy_pos); j++){
            ...
            int emp_row = (empty_row_count[max_r] - empty_row_count[min_r - 1]);
            int emp_col = (empty_column_count[max_c] - empty_column_count[min_c - 1]);
            ans += (
                ((ll)(max_r - min_r) + (ll)emp_row * (ll)999999) + 
                ((ll)(max_c - min_c) + (ll)emp_col * (ll)999999)
            );
        }
    }
    cout << ans;
